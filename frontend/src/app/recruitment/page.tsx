...
import { AnalyticsService } from '@/services/api/recruitment.service';
    ...
    const fetchData = async () => {
        try {
            const [
                ...
                AnalyticsService.getProgress().catch(() => ({ data: null })),
                AnalyticsService.getHiringFunnel().catch(() => ({ data: null })),
                AnalyticsService.getSourceEffectiveness().catch(() => ({ data: null })),
                AnalyticsService.getTimeToFill().catch(() => ({ data: null })),
                AnalyticsService.getOffers().catch(() => ({ data: null }))
            ]);

            if (progressRes?.data) setProgress(progressRes.data);
            if (funnelRes?.data) setFunnel(funnelRes.data);
            if (sourceRes?.data) setSources(sourceRes.data);
            if (timeRes?.data) setTimeToFill(timeRes.data);
